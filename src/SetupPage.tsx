...
const toolCategories: ToolCategory[] = [
  ...
  },
  {
    ...
    tools: [
      {
        id: 'vpn-install-dotycat', name: 'Dotycat Tunnel', icon: '\u{1F431}',
        description: 'VLESS/VMess/Trojan WS/gRPC/xHTTP + SSH WS + OpenVPN. Active 2026, 28 stars.',
        script: 'apt update && apt upgrade -y && apt install -y git wget curl unzip && wget -O /root/install.sh https://raw.githubusercontent.com/dotywrt/doty/main/install.sh && chmod +x /root/install.sh && /root/install.sh',
        warning: 'This will install a full VPN autoscript! Only run on a FRESH VPS. If you already have another autoscript installed, rebuild your VPS first.',
      },
      {
        id: 'vpn-install-vinstech-lite', name: 'Vinstech Lite', icon: '\u{26A1}',
        description: 'VMess/VLess/Trojan WS & gRPC. Popular in MY. 30 stars.',
        script: 'sysctl -w net.ipv6.conf.all.disable_ipv6=1 && sysctl -w net.ipv6.conf.default.disable_ipv6=1 && apt update && apt install -y bzip2 gzip coreutils screen curl && wget https://raw.githubusercontent.com/vinstechmy/AutoscriptWebsocketLite/main/V1/setup-lite.sh && chmod +x setup-lite.sh && screen -S vinstech ./setup-lite.sh',
        warning: 'This will install a full VPN autoscript! Only run on a FRESH VPS. Requires domain pointed to VPS IP.',
      },
      {
        id: 'vpn-install-vinstech-minix', name: 'Vinstech MiniXLite', icon: '\u{26A1}',
        description: 'VLess/Trojan WS + TCP XTLS. Trial accounts, Telegram backup. 16 stars.',
        script: 'sysctl -w net.ipv6.conf.all.disable_ipv6=1 && sysctl -w net.ipv6.conf.default.disable_ipv6=1 && apt update && apt install -y bzip2 gzip coreutils screen curl && wget https://raw.githubusercontent.com/vinstechmy/MiniXLiteAutoscript/main/V1/setup.sh && chmod +x setup.sh && screen -S vinstech ./setup.sh',
        warning: 'This will install a full VPN autoscript! Only run on a FRESH VPS. Requires domain pointed to VPS IP.',
      },
      {
        id: 'vpn-install-vinstech-multi', name: 'Vinstech Multiport', icon: '\u{26A1}',
        description: 'SSH WS + VMess/VLess/Trojan WS multiport. 8 stars.',
        script: 'sysctl -w net.ipv6.conf.all.disable_ipv6=1 && sysctl -w net.ipv6.conf.default.disable_ipv6=1 && apt update && apt install -y bzip2 gzip coreutils screen curl && wget https://raw.githubusercontent.com/vinstechmy/SSH-XRAY-Websocket-Multiport/main/setup.sh && chmod +x setup.sh && screen -S vinstech ./setup.sh',
        warning: 'This will install a full VPN autoscript! Only run on a FRESH VPS. Requires domain pointed to VPS IP.',
      },
      {
        id: 'vpn-install-decode', name: 'Decode Reality', icon: '\u{1F50D}',
        description: 'VLess-only (Reality). TLS/gRPC/HttpUpgrade/xHTTP. Clean & focused. New 2026.',
        script: 'wget -q https://raw.githubusercontent.com/DecodeXOfficial/Reality/main/setup.sh -O setup.sh && chmod +x setup.sh && screen -S decode ./setup.sh',
        warning: 'This will install a full VPN autoscript! Only run on a FRESH VPS. VLess protocol only.',
      },
      {
        id: 'vpn-install-rerechan', name: 'FN Project (Rerechan)', icon: '\u{1F30D}',
        description: 'VMess/VLess/Trojan + NoobzVPN + SlowDNS + UDP Custom. ALL OS support. 23 stars.',
        script: 'apt update && apt install wget curl screen gnupg openssl perl binutils -y && wget -O install.sh "https://codeberg.org/Rerechan02/scvps-stable/raw/branch/main/install.sh" && chmod +x install.sh && screen -S fn ./install.sh',
        warning: 'This will install a full VPN autoscript! Only run on a FRESH VPS. Supports ALL Linux distros.',
      },
      {
        id: 'vpn-install-darqan', name: 'DarQan Script', icon: '\u{1F4DC}',
        description: 'SSH WS + VLess WS only. IP limit, data limit, user recovery. Debian 11/12.',
        script: 'apt update ; apt install wget curl openssl perl screen -y ; wget -q https://raw.githubusercontent.com/darul-itqan/Auto-Script-VPS-SSH-WS-VLESS-WS/main/install.sh ; chmod +x install.sh ; screen -S rere ./install.sh',
        warning: 'This will install a full VPN autoscript! Only run on a FRESH VPS (Debian 11/12). Run apt full-upgrade -y && reboot first.',
      },
      {
        id: 'vpn-install-gegevps', name: 'GegeVPS', icon: '\u{1F5A5}\u{FE0F}',
        description: 'SSH/OpenVPN/Xray/WireGuard + SlowDNS + UDP Custom. Cloudflare auto-DNS. 54 stars.',
        script: 'apt update && apt install -y wget curl screen && wget https://raw.githubusercontent.com/GegeDevs/sshvpn-script/main/setup.sh && chmod +x setup.sh && screen -S gege ./setup.sh',
        warning: 'This will install a full VPN autoscript! Only run on a FRESH VPS. Requires Cloudflare domain.',
      },
      {
        id: 'vpn-install-kingkong', name: 'KingKongVPN', icon: '\u{1F98D}',
        description: 'SSH/OpenVPN/Stunnel/Xray Multiport/WireGuard. Webmin panel. 163 stars.',
        script: 'wget https://raw.githubusercontent.com/xiihaiqal/AutoScriptVPS/master/AutoScript && bash AutoScript',
        warning: 'This will install a full VPN autoscript! Only run on a FRESH VPS (Debian 10/Ubuntu 18-20).',
      },
      {
        id: 'vpn-install-netzxray', name: 'Netz-Xray (110 Contributors)', icon: '\u{1F465}',
        description: 'Xray only: VMess/VLess/Trojan/SS WS & gRPC. Dynamic path. 142 stars.',
        script: 'wget -q https://raw.githubusercontent.com/adisubagja/AutoScriptXray/master/adi.sh && chmod +x adi.sh && screen -S netzinstall ./adi.sh',
        warning: 'This will install a full VPN autoscript! Only run on a FRESH VPS (Debian 10/Ubuntu 20).',
      },
      {
        id: 'vpn-install-givpn', name: 'GIVPN', icon: '\u{1F512}',
        description: 'VMess/VLess/Trojan/SS WS & gRPC + SSH WS + OpenVPN. 49 stars.',
        script: 'apt update && apt install -y bzip2 gzip coreutils screen curl unzip && wget https://raw.githubusercontent.com/givpn/AutoScriptXray/master/setup.sh && chmod +x setup.sh && screen -S givpn ./setup.sh',
        warning: 'This will install a full VPN autoscript! Only run on a FRESH VPS.',
      },
      {
        id: 'vpn-install-farelvpn', name: 'FarellVPN (Modern)', icon: '\u{1F4BB}',
        description: 'VMess/VLess/Trojan + Web API + Telegram bot + Quota management. Python+Go.',
        script: 'apt update && apt install -y wget curl screen && wget https://raw.githubusercontent.com/farelvpn/autoscript/main/setup.sh && chmod +x setup.sh && screen -S farel ./setup.sh',
        warning: 'This will install a full VPN autoscript! Only run on a FRESH VPS. Has REST API for remote management.',
      },
      {
        id: 'vpn-install-mantap', name: 'SL Mantap AIO', icon: '\u{1F4AA}',
        description: 'SSH/OHP/Stunnel5/OpenVPN/Xray/SS/SSR/WireGuard/Trojan-Go. 209 stars.',
        script: 'wget https://raw.githubusercontent.com/fisabiliyusri/Mantap/main/setup.sh && chmod +x setup.sh && ./setup.sh',
        warning: 'This will install a full VPN autoscript! Only run on a FRESH VPS (Debian 9-10/Ubuntu 18-20).',
      },
      {
        id: 'vpn-install-praiman', name: 'PR Aiman AIO (13+ Protocols)', icon: '\u{1F3C6}',
        description: 'SSH/OpenVPN/Xray/Trojan/SS/SSR/WireGuard/SSTP/L2TP/PPTP. Most protocols.',
        script: 'apt install -y bzip2 gzip coreutils curl && wget https://raw.githubusercontent.com/praiman99/AutoScriptVPN-AIO/Beginner/setup.sh && chmod +x setup.sh && ./setup.sh',
        warning: 'This will install a full VPN autoscript! Only run on a FRESH VPS (Debian 9-10).',
      },
      {
        id: 'vpn-install-scvps', name: 'SCVPS AIO', icon: '\u{1F5A5}\u{FE0F}',
        description: 'SSH/Xray/WireGuard/Trojan-Go/SSR/L2TP. Theme menu, admin panel. 92 stars.',
        script: 'apt update && apt upgrade -y && apt install -y wget screen && wget -q https://raw.githubusercontent.com/scvps/scriptvps/main/setup.sh && chmod +x setup.sh && screen -S setup ./setup.sh',
        warning: 'This will install a full VPN autoscript! Only run on a FRESH VPS (Debian 10/Ubuntu 18-20).',
      },
      {
        id: 'vpn-install-caliph', name: 'Caliph Dev', icon: '\u{1F3F0}',
        description: 'Xray (VMess/VLess/Trojan/SS) WS & gRPC + SSH WS. No IP registration. 7 stars.',
        script: 'apt update && apt install -y wget curl screen && wget https://raw.githubusercontent.com/cabrata/autoscript/master/setup.sh && chmod +x setup.sh && screen -S caliph ./setup.sh',
        warning: 'This will install a full VPN autoscript! Only run on a FRESH VPS (Debian 11-12/Ubuntu 20-22).',
      },
      {
        id: 'vpn-install-233boy', name: '233boy Xray (2.2K Stars)', icon: '\u{2B50}',
        description: 'VLESS Reality/VMess/Trojan/SS2022. Best CLI manager. Chinese community.',
        script: 'bash <(wget -qO- https://raw.githubusercontent.com/233boy/Xray/main/install.sh)',
        warning: 'This will install Xray with CLI management. Works on existing VPS. Manage via "xray" command after install.',
      },
      {
        id: 'vpn-install-jinwyp', name: 'One Click Script (5.1K Stars)', icon: '\u{1F31F}',
        description: 'V2Ray/Xray/Trojan-Go/WireGuard/SS + BBR kernel. Most starred. Chinese community.',
        script: 'wget -O setup.sh https://raw.githubusercontent.com/jinwyp/one_click_script/master/install_v2ray.sh && bash setup.sh',
        warning: 'This will install a full VPN stack! Interactive menu. Works on existing VPS but may conflict with other scripts.',
      },
      {
        id: 'vpn-install-afandiazmi', name: 'afandiazmi 8-in-1', icon: '\u{1F4E6}',
        description: 'VLESS/VMess/Trojan 8 combo (TCP/WS/gRPC + TLS/XTLS). Xray & V2Ray core. 39 stars.',
        script: 'apt update && apt install -y wget curl screen && wget https://raw.githubusercontent.com/afandiazmi/v2RayVPN/main/setup.sh && chmod +x setup.sh && screen -S afandi ./setup.sh',
        warning: 'This will install a full VPN autoscript! Only run on a FRESH VPS.',
      },
      {
        id: 'vpn-install-senovpn', name: 'SenoVPN', icon: '\u{1F5A5}\u{FE0F}',
        description: 'SSH/OHP/OpenVPN/Stunnel5/Xray/SSR/WireGuard/Trojan-Go/SSTP/L2TP/PPTP. 61 stars.',
        script: 'rm -f setup.sh && sysctl -w net.ipv6.conf.all.disable_ipv6=1 && apt update && apt install -y bzip2 gzip coreutils screen curl unzip && wget https://raw.githubusercontent.com/senowahyu62/scriptvps/main/setup.sh && chmod +x setup.sh && ./setup.sh',
        warning: 'This will install a full VPN autoscript! Only run on a FRESH VPS (Debian 10/Ubuntu 18-20).',
      },
      {
        id: 'vpn-install-rascom', name: 'RasCom AIO', icon: '\u{1F4E1}',
        description: 'SSH/OpenVPN/V2Ray/Trojan/Trojan-Go/SS/SSR/WireGuard/SSTP/L2TP/PPTP. Telegram bot. 13 stars.',
        script: 'apt update && apt install -y wget curl screen && wget https://raw.githubusercontent.com/myskynblack/aioscvps/main/setup.sh && chmod +x setup.sh && screen -S rascom ./setup.sh',
        warning: 'This will install a full VPN autoscript! Only run on a FRESH VPS (Debian 9-10/Ubuntu 18-20).',
      },
      {
        id: 'vpn-install-givps-tor', name: 'GIVPS + Tor', icon: '\u{1F575}\u{FE0F}',
        description: 'Xray/SSH WS/Stunnel/OpenVPN + Tor integration for anonymity. 16 stars.',
        script: 'apt update && apt install -y bzip2 gzip coreutils screen curl unzip && wget https://raw.githubusercontent.com/givps/AutoScriptXray/master/setup.sh && chmod +x setup.sh && screen -S setup ./setup.sh',
        warning: 'This will install a full VPN autoscript! Only run on a FRESH VPS (Debian 11-12/Ubuntu 18-22).',
      },
    ],
  },
  {
